    ...
    <div className="divide-y divide-slate-200">
      {items.map((notification) => (
        <Link
          ...
          className={`flex items-start gap-3 px-5 py-4 transition hover:bg-slate-50 sm:px-6 ${compact ? "pr-4 sm:pr-5" : ""}`}
        >
          <div className="pt-1">
            <span
              className={`block h-2.5 w-2.5 rounded-full ${notification.isRead ? "bg-slate-200" : "bg-student"}`}
              aria-hidden="true"
            />
          </div>
            ...
            <div className="relative">
              ...
              >
                ...
                {unreadNotificationCount > 0 ? (
                  <span className="absolute -right-1 -top-1 inline-flex min-h-5 min-w-5 items-center justify-center rounded-full bg-student px-1.5 text-[11px] font-semibold text-white">
              ...
              {notificationsOpen ? (
                <>
                  ...
                  <div className="absolute right-0 z-20 mt-3 w-[24rem] overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl shadow-slate-900/12">
                    ...
                    <NotificationList items={notifications.slice(0, 4)} compact />
                  </div>
                </>
              ) : null}
            </div>
